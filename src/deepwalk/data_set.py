...
class GraphData(object):
    def __init__(self, config, rebuild=False, logger_info=print):
        ...
        注: 代码参考gcn的data_set.py, 部分冗余代码没有删除。
        ...
        self.data_root = config['data_path']    # 处理数据集路径
        self.logger_info = logger_info          # 日志函数
        
        
        if self.data_root in ['cora', 'citeseer', 'pubmed']:
            # 文件名称
            self.filenames = ["ind.{}.{}".format(self.data_root, name) for name in
                    ['x', 'tx', 'allx', 'y', 'ty', 'ally', 'graph', 'test.index']]
            # 名称转路径
            self.data_root = os.path.join(self.get_project_path(), 'data', self.data_root)
            # 缓存数据路径
            if self.use_semi_supervised:
                self.save_file = os.path.join(self.data_root, "deepwalk_processed_data_semi.pkl")
            else:
                self.save_file = os.path.join(self.data_root, "deepwalk_processed_data_sup.pkl")
    ...
    def get_data(self):
        """
        获取数据, 包括 标签、训练集、验证集、测试集, 邻接矩阵

        Return:
            tensor_y: torch.tensor
            tensor_train_mask: torch.tensor
            tensor_val_mask: torch.tensor
            tensor_test_mask: torch.tensor
            adjacency: np.int32
        """
        # 读取数据并预处理
        ...
        node_feature = dataset['x']
        tensor_y = self.to_tensor(dataset['y']) # 节点标签，cora=(2708,)
        _, input_dim = node_feature.shape       # 节点特征维度, cora=(2708,1433)
        ...
        return tensor_y, tensor_train_mask, tensor_val_mask, tensor_test_mask, adjacency
...
if __name__=="__main__":
    ...
    config = {'data_path':'citeseer',"use_semi_supervised":True, 'device':'cpu'}
    a = GraphData(config=config, rebuild=True)
    y, train_mask, val_mask,test_mask, adjacency = a.get_data()
